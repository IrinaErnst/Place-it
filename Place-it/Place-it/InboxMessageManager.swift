...
class InboxMessageManager: NSObject {
    ...
    override init(){
        var tempMessages: NSArray = persistenceHelper.list("InboxMessage") // Reference data model
        
        for res:AnyObject in tempMessages{
            if (res.valueForKey("receiver") as String == myPhoneNumber) {
    ...
    func addMessage(sender_arg: String, receiver_arg: String, place_arg: String, time_arg: String, content_arg: String, timeOfCreating_arg: String, ID_arg: String){
        ...
        if(persistenceHelper.save("InboxMessage", parameters: dicMessage)){
    ...
    func removeMessage(index:Int) -> message{
        ...
        if(persistenceHelper.remove("InboxMessage", key: "id", value: value)){
