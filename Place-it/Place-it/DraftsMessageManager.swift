...
class DraftsMessageManager: NSObject {
    ...
    override init(){
        var tempMessages: NSArray = persistenceHelper.list("DraftsMessage") // Reference data model
    ...
    func addMessage(sender_arg: String, receiver_arg: String, place_arg: String, time_arg: String, content_arg: String, timeOfCreating_arg: String, ID_arg: String){
        ...
        if(persistenceHelper.save("DraftsMessage", parameters: dicMessage)){
    ...
    func removeMessage(index:Int) -> message{
        ...
        if(persistenceHelper.remove("DraftsMessage", key: "id", value: value)){
